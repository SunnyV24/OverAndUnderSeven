...
beginGame_ans = questdlg(['Welcome to Over 7 and Under 7! ' ...
    ...
    'Start','Start Game!','Instruction','Cancel');
if(strcmpi(beginGame_ans,'Instruction'))
    msgbox(["Instruction for Over 7 and Under 7!:";"1. The player is given a initial betting amount of $100."]);
end
player_bet = getBet();
...
%Winning/Losing Conditions
    ...
    rst = strcat("You Rolled a total of " + sum(roll) +"! Current balance is " + sprintf('$%.2f',player_money));
    text(275,150,rst,VerticalAlignment="bottom",FontSize=10,HorizontalAlignment="right");
    
    if(player_money==0)
        drawScene(bet_scene,[skull_sprite,skull_sprite]);
        opts.Interpreter = 'tex';
        replay_ans = questdlg('Would you like to play again?','Play Again?','Yes','No','Cancel');
        if(strcmpi(replay_ans,'Yes'))
            SevenandUnder
        end
    end
    opts.Interpreter = 'tex';
    reroll_ans = questdlg('Would you like to bet again?','Bet Again?','Yes','No','Cancel');
